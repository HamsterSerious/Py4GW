...
class BuildValidator:
    ...
    def test_player_loadout(self, build_code: str, expected_skills: int = 8):
        """
        ...
        yield from Routines.Yield.wait(Timing.SKILL_VERIFY_DELAY)
    ...
    def test_mandatory_heroes(self, hero_requirements: list, mission_name: str,
                              get_assigned_hero_fn, get_hero_name_fn):
        """
        Tests loading all mandatory heroes with their builds.
        
        Args:
            hero_requirements: List of HeroRequirements
        ...
        for idx, req in enumerate(hero_requirements):
            ...
            party_slot = yield from self._wait_for_hero(hero_id, timeout=Timing.HERO_APPEAR_TIMEOUT)
            if party_slot == -1:
                Py4GW.Console.Log(
                    "BuildValidator", 
    ...
    def verify_hero_build(self, party_slot: int, expected_count: int = 8) -> tuple:
        """
    ...
    def _wait_for_hero(self, hero_id: int, timeout: float = 3.0):
        """
    ...
    def _load_and_verify_hero_build(self, party_slot: int, build_code: str,
                                     expected_skills: int, hero_name: str):
        """
    ...
    def _count_loaded_skills(self, hero_data) -> int:
        """
    ...
    def _extract_skill_id(self, item) -> int:
        """
