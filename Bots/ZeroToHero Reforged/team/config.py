...
from data.heroes import get_hero_display_name, get_mercenary_hero_ids
...
class TeamConfigManager:
    ...
    def initialize(self) -> bool:
        ...
        # Setup paths
        # Path: team/config.py -> team -> ZeroToHero (2 dirname calls)
    ...
    def load(self):
        ...
        try:
            ...
            # Load profiles (handle legacy format)
            if "profiles" in data:
    ...
    def get_profile(self, party_size: int, mode: str) -> dict:
        """
    ...
    def set_profile(self, party_size: int, mode: str, profile: dict):
        """Updates a profile."""
        key = f"{party_size}_{mode}"
    ...
    def get_assigned_hero(self, mission_name: str, slot_index: int, default_hero_id: int = 0) -> int:
    ...
    def set_assigned_hero(self, mission_name: str, slot_index: int, hero_id: int) -> bool:
        """
    ...
    def _populate_default_merc_names(self):
        ...
        if not self.custom_hero_names:
            for i, hero_id in enumerate(get_mercenary_hero_ids(), start=1):
                self.custom_hero_names[str(hero_id)] = f"Mercenary {i}"
    
    def _convert_legacy_profiles(self, legacy_profiles: dict) -> dict:
        """
    ...
    def _sanitize_profiles(self, profiles: dict) -> dict:
